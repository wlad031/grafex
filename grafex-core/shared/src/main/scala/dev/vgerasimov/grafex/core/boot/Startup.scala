...
/** ADT which describes the "startup mode" of the application.
  ...
  *   1. when `--help` flag specified, the help message will be printed to STDOUT
  *      and then application will exit with code = 0;
  *   1. when at least one mode call specified, application will process it, print the result to STDOUT
  *      and exit with code = 0, or print the error to STDERR and exit with code = 1;
  *   1. when `service` subcommand and at least one listener specified, application will be processing incoming
