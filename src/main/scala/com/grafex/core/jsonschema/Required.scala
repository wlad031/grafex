...
object Required {
  ...
  object Fields {
    ...
    implicit def hlistFields[K <: Symbol, H, T <: HList](
      ...
      if (req.value.isRequired) witness.value.name :: tFields.get
      else tFields.get
    }
    ...
    implicit def genericFields[A, R](implicit gen: LabelledGeneric.Aux[A, R], rfs: Fields[R]): Fields[A] =
      instance(rfs.get)
